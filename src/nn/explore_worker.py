...
class ExploreWorker(mp.Process):
    def __init__(self, game_states: list, frames_per_evaluation: int, reaction_delay: int, env_status: dict,
        ...
        """
        super(ExploreWorker, self).__init__()
            ...
            while not self.eval_status['kill_eval']:
                ...
                while not self.env_status['round_done'] and not self.eval_status['kill_eval']:
                    ...
                    if last_normalized_index > last_evaluated_index:
                        ...
                        detached_out = torch.zeros(self.input_index_max)

                        if self.states[-1]['game'][self.player_idx]['motion_type'] == 0:
                            explore = 1
                            if self.model_config['input_mask'] is None:
                                action_index = random.randrange(self.input_index_max)  # select random action
                            else:
                                action_index = random.choice(self.model_config['input_mask'])
                        else:
                            explore = 0
